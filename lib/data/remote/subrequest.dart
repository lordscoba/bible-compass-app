...
class SubNotifier extends StateNotifier<SubscriptionState> {
  ...
  Future<void> perfromPayStackInitRequest(dynamic ref, String userId) async {
    try {
      // Set loading state
      ...
      // debugPrint(EnvironmentSubConfig.paystackSubInit + userId);
      // debugPrint(ref.toString());

      // Make the POST request
      final response = await dio
          .post(EnvironmentSubConfig.paystackSubInit + userId, data: ref);
  ...
  Future<SubscriptionState> perfromGetVerifySubRequest(String rid) async {
    try {
      // Set loading state
      state = state.copyWith(isLoading: true, error: '');
      final dio = Dio();
      ...
      // Make the POST request
      final response =
          await dio.get(EnvironmentSubConfig.paystackSubVerify + rid);
      if (response.statusCode == 200 || response.statusCode == 201) {
        // Request successful
        state = state.copyWith(
            isLoading: false,
